...
The asset designer's **Scaled variants...** button manages alternate resolutions of the current source version. Set width and height (per frame for animations/sheets, per tile for tilesets), then **Generate**. **Edit** changes the dimensions and offers **Regenerate**; **Touch up…** opens the existing pixel editor; **Delete** removes a variant from use. Changes save immediately. The source image, logical asset dimensions, frame order, and animation timing remain unchanged. A newly promoted source starts with its own variants.

Generation selects the closest available resolution by scale ratio, preferring the larger source on a tie. Regeneration excludes the variant being replaced. Strict pixels uses exact nearest-neighbor RGBA copying. Smooth resize uses deterministic resampling. AI super-resolution uses a dedicated upscaling provider, without image-generation prompts; it can infer detail. Downscaling in that mode uses deterministic smooth resizing. Transparent coverage is preserved and sheets are processed one cell at a time.
