...
export function createDesignerElements(
...
export function assetCrumbButton(
  label: string,
  pathParts: string[],
  options: {
    container: HTMLDivElement;
...
export function folderPathForAsset(manifest: AiAssetManifest, assetId: string): string[] {
...
export function pathHasAnyAsset(
  pathParts: string[],
...
export function pickUploadFile(accept: string): Promise<File | undefined> {
...
export function imageSizeFromSource(src: string): Promise<AiAssetPreviewDisplaySize> {
  return new Promise((resolve, reject) => {
    const image = new Image();
...
export function touchUpButton(label: string): HTMLButtonElement {
  const button = document.createElement("button");
  button.type = "button";
  button.textContent = label;
...
export function audioFormatFromMimeType(mimeType: string, fileName: string): AiAudioFormat {
...
export function normalizeAssetFormatFromMimeType(mimeType: string, fileName: string): AiAssetFormat {
...
export function seekableAudioTime(state: AudioTrimState, progress: number): number {
...
export function resetAudioPlayerContainer(container: HTMLDivElement): void {
...
export function pauseSiblingAudioPlayers(container: HTMLElement): void {
...
export async function audioWaveformPeaks(src: string, bucketCount = 96): Promise<AudioWaveformPeaks> {
...
export function drawAudioEditorWaveform(
  canvas: HTMLCanvasElement,
  options: {
    peaks: AudioWaveformPeaks | undefined;
    duration: number;
...
export function progressForAudio(audio: HTMLAudioElement): number {
...
export function setFrameElementBackground(
...
export function generationOverridesFromInputs(
  elements: DesignerElements,
  asset: AiAssetDefinition,
...
export function audioGenerationOverridesFromInputs(
  elements: DesignerElements,
  asset: AiAssetDefinition
...
export function canEditGenerationFormat(
  manifest: AiAssetManifest,
  selectedAssetId: string,
...
export function effectiveGenerationFormat(
  manifest: AiAssetManifest,
...
export function normalizeAssetFormat(format: string | undefined): AiAssetFormat {
  if (format === "jpg" || format === "webp" || format === "svg") return format;

  return "png";
}

export function normalizeAudioFormat(format: string | undefined): AiAudioFormat {
...
export function isAudioAsset(asset: AiAssetDefinition | undefined): boolean {
...
export function integerInput(input: HTMLInputElement, fallback: number): number {
  const value = Number(input.value);
...
export function numberInput(input: HTMLInputElement, fallback: number): number {
  const value = Number(input.value);
...
export function inlineCheckboxField(label: string, checkbox: HTMLInputElement): HTMLLabelElement {
...
export function positiveIntegerValue(value: number | undefined, fallback: number): number {
  if (!Number.isFinite(value)) return fallback;

  return Math.max(1, Math.floor(value as number));
}

export function integerValue(value: number | undefined, fallback: number): number {
  if (!Number.isFinite(value)) return fallback;

  return Math.trunc(value as number);
}

export function numberValue(value: number | undefined, fallback: number): number {
  if (!Number.isFinite(value)) return fallback;
...
export function labelWrap(labelText: string, control: HTMLElement): HTMLLabelElement {
...
export function numericInput(): HTMLInputElement {
  const input = document.createElement("input");
  input.type = "number";
  input.min = "1";
  input.step = "1";
...
export function signedNumberInput(): HTMLInputElement {
  const input = document.createElement("input");
  input.type = "number";
  input.step = "1";
  input.inputMode = "numeric";
...
export function decimalInput(): HTMLInputElement {
  const input = document.createElement("input");
  input.type = "number";
  input.step = "any";
...
export function hasStyleGuide(styleGuide: StyleGuideDraft): boolean {
  return Boolean(styleGuide.prompt.trim() || styleGuide.images.length);
}

export async function styleGuideRequest(styleGuide: StyleGuideDraft): Promise<DebugStyleGuideDraft> {
  return {
...
export function readableAssetName(assetId: string): string {
...
export function stopStatusAnimation(status: HTMLDivElement): void {
...
export function ensureDesignerStyles(): void {
